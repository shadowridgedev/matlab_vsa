...
function processNormal(hObject, eventdata, handles)
    [input,Fs] = audioread('recordings/numbers_01-1_norm.wav');
    ...
    plot(impulseCT,'r');
    hold on
    plot(impulseOT,'b'),title('Voiced Overlay Normal Signal','FontSize',8);
    ...
    jit = jitter(difference);
    set(handles.textJitterNormal, 'String', num2str(round(jit)));
...
function processStressed(hObject, eventdata, handles)
    [input,Fs] = audioread('recordings/numbers_01-1_stress.wav');
    
    %input(abs(input) < 0.05) = 0;
    ...
    plot(impulseCT,'r');
    hold on
    plot(impulseOT,'b'),title('Voiced Overlay Stressed Signal','FontSize',8);
    axis([0 20000 -0.4 0.4]);
    ...
    jit = jitter(difference);
    set(handles.textJitterStressed, 'String', num2str(round(jit)));
    

 
    
function jit = jitter(diffVector)

    s = length(diffVector);
    diffSum = sum(diffVector);
    jit = diffSum/(s-1);
